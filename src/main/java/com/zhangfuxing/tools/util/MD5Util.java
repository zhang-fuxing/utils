...
public class MD5Util {
    ...
    public static String encode(String source, Charset charset, Boolean isUpperCase) {
        String md5;
        try {
            md5 = Str.toMD5(source, charset, isUpperCase);
    ...
    public static String encode(File file, Boolean isUpperCase, long size) throws IOException {
        if (file == null || !file.exists()) return "";
        InputStream is = new FileInputStream(file);
        return encode(is, isUpperCase, size);
    }

    public static String encode(File file, long size) throws IOException {
    ...
    public static String encode(InputStream inputStream, Boolean isUpperCase, long size) throws IOException {
        if (inputStream == null) return "";
        try (inputStream) {
            byte[] buf;
            if (size <= 0) buf = new byte[1 << 12];
            else buf = new byte[(int) size];
            int len = inputStream.read(buf);
            char[] chars = Str.encode(getMd5Bytes(buf), isUpperCase);
    ...
    public static String encode(InputStream inputStream, long size) throws IOException {
