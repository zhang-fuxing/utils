...
public class ConvertUtil {
	...
	public static Short toShort(String source) {
		Short convert = null;
		try {
			convert = convert(source, t -> Str.isBlank(source) ? null : Short.parseShort(t));
	...
	public static Integer toInt(String source) {
		...
		try {
			convert = convert(source, t -> Str.isBlank(source) ? null : Integer.parseInt(t));
		} catch (Exception ignored) {}
		return convert;
	}
	...
	public static Long toLong(String source) {
		Long convert = null;
		try {
			convert = convert(source, t -> Str.isBlank(source) ? null : Long.parseLong(t));
		} catch (Exception ignored) {}
		return convert;
	}
