...
public class DateUtil {
	...
	public static Long timestamp(String offsetId) {
		return LocalDateTime.now().toEpochSecond(ZoneOffset.of(offsetId));
	}
	
	// 时间偏移， off为正，加多少时间，负，减多少时间
	public static Date offsetYear(Date date, int off) {
		return offset(date, off,Y);
	}
	...
	public static Date offset(Date date, int off, int model) {
		LocalDateTime temp = dateToLocalDateTime(date);
		temp = switch (model) {
			case S -> temp.plusSeconds(off);
			case M -> temp.plusMinutes(off);
			case H -> temp.plusHours(off);
			case D -> temp.plusDays(off);
			case L -> temp.plusMonths(off);
			case Y -> temp.plusYears(off);
			default -> throw new IllegalArgumentException("未知的偏移模式");
		};
